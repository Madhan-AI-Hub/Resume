...
const isRelevantDocument = (text, type = 'resume') => {
    const resumeMarkers = ['experience', 'education', 'skills', 'projects', 'summary', 'certification', 'employment', 'university', 'college'];
    const jdMarkers = ['responsibilities', 'requirements', 'qualifications', 'preferred', 'required', 'description', 'opportunity', 'stack'];
    const markers = type === 'resume' ? resumeMarkers : jdMarkers;
    const count = markers.reduce((acc, marker) => acc + (new RegExp(`${marker}`, 'i').test(text) ? 1 : 0), 0);
    return count >= 1; // More permissive for relevance
};
...
const extractSkills = (text) => {
    ...
    techLexicon.forEach(skill => {
        if (new RegExp(`\\b${skill.replace('.', '\\.')}\\b`, 'i').test(text)) {
